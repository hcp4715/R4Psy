...
# ---------- 1.3 重复测量方差分析 ----------
## 载入数据 并进行变量预处理
mt_raw <- bruceR::import(here::here('data','match','match_raw.csv')) %>% #使用here包定位该文件，improt包输出csv文件内容，最后的管道符是将这一步操作传递给下一步
      tidyr::extract(Shape,                                  #tidyr::extract()函数对数据框中的Shape列进行字符串提取
                     into = c("Valence", "Identity"),        #创建两个新列分别命名为"Valence"和"Identity"
                     regex = "(moral|immoral)(Self|Other)",  #第一个括号组(moral|immoral)匹配"moral"或"immoral"，第二个同理
                     remove = FALSE)                         #保留原始的shapelie不删除

# 分被试和实验条件计算 Match条件下正确反应试次的RT均值
mt_mean <- mt_raw %>%                                               #使用dplyr包对数据框mt_raw进行了一系列数据处理操作，最终生成一个新的数据框mt_mean
      dplyr::filter(!is.na(RT) & Match == "match" & ACC == 1) %>%    #筛选行，保留满足以下所有条件：RT列没有缺失值，Match列=match，ACC列=1
      dplyr::group_by(Sub,Identity,Valence) %>%                     #按照三列分组
      dplyr::summarise(RT = mean(RT)) %>%                           #计算每组RT的平均值并将数据保存在meanRT列
      dplyr::ungroup()                                              #移除分组，返回普通的数据库
...
m_aov <- afex::aov_ez(
      ...
      id = 'Sub',
      dv = 'RT',
      within = c('Identity', 'Valence'))   #自变量是什么
# 打印afex的结果
m_aov
