...
<br>
<br>
<span style="font-size: 45px;">回顾：问卷数据</span></center> <br>
<br>
<span style="font-size: 45px;">6.1 Tidyverse</span></center> <br>
<br>
<span style="font-size: 45px;">6.2 基于Tidyverse处理问卷数据</span></center> <br>
<span style="font-size: 30px;">&emsp;6.2.1 操作步骤</span></center> <br>
<span style="font-size: 30px;">&emsp;6.2.2 小结</span></center> <br>
<br>
<span style="font-size: 45px;">6.3 基于Tidyverse处理反应时数据</span></center> <br>
...
---
# <h1 lang="en">回顾：研究问题</h1>
<br>
- 课程接下来会以重复[IJzerman et al (2018)](https://doi.org/10.1525/collabra.165)的分析来回顾此前的问卷数据分析流程，并学习基于 Tidy verse的流程<br>
<br>
...
&emsp;&emsp; ALEX: 述情障碍，探索性监督机器学习需要的变量之一

&emsp;&emsp;&emsp;&emsp; (5点量表，变量为ALEX1-16，第4, 12, 14, 16题反向计分)
...
---
# <h1 lang="en">回顾：操作步骤</h1>
- 数据预处理目标：计算感兴趣的变量，按照Site查看被试的平均体温<br>

![process_penguin1](picture/chp6/process_penguin1.png)

&emsp;&emsp; Step1: 选择变量<br>
&emsp;&emsp; Step2: 已有变量处理(检查数据类型、处理缺失值、<font color='red'>反向计分</font>) <br>
...
![process_penguin2](picture/chp6/process_penguin2.png)
---
# <h1 lang="en">回顾：以反向计分为例</h1>
<span style="font-size: 30px;">例子 | 反向计分[bruceR::SUM函数]</span></center><br>
```{r, eval=FALSE}
# 导入包
...
```
<br>
- bruceR的操作会产生新的中间变量(vars_to_reverse)
![output](picture/chp6/output.png)
---
...
- 本课程的数据预处理将基于tidyverse( https://www.tidyverse.org/ )，它是目前最流行的预处理工具，是由9个核心R包和10个附加R包组成的连贯的系统<br>
- 核心包的具体功能与工作流如图所示
<!-- ##### -->
<!-- ##### -->
...
<!-- ##### -->
<!-- ##### -->
<!-- 我查了一下，可能因为新版本涵盖的包较多，官方提供的综合cheatsheet中包含了大量与数据处理本身无关的内容（见posit-team1.png），因此这里还是保留了之前的版本，或者可以像这页一样举例展现一个包的cheatsheet-->
<!-- ##### -->
<!-- ##### -->
# <h1 lang="en">6.1 Tidyverse</h1>
<img src="./picture/chp6/cheatsheet_forcat.png" width="75%" style="display: block; margin-left: auto; margin-right: auto;">
- 可以通过搜索包的cheatsheet快速了解包的使用(此处以“forcat”包为例)<br>
- 官方cheatsheet: https://rstudio.github.io/cheatsheets/
...
---
# <h1 lang="en">6.1 Tidyverse</h1>
<span style="font-size: 30px;">例子 | 反向计分[mutate,case_when]</span></center><br>
```{r}
# 导入数据
df2 <- bruceR::import(here::here('data', 'penguin', 'penguin_rawdata.csv'))
...
&emsp;&emsp; Step4: 反向计分(已完成) [mutate, case_when]<br>
...
df2 <- filter(df2, !is.na(Temperature_t1) 
...
df2 <- dplyr::mutate(df2, 
                     Temperature = rowMeans(select(df2, starts_with("Temperature"))))
```
...
# 计算ALEX
df2 <- mutate(df2, 
              ALEX = rowSums(select(df2, starts_with("ALEX"))))
