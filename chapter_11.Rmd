...
---
## <h1 lang="en">fill = interaction()</h1> 
...
names(p)[16] <- "Layers and Facets Full Codes"; p[[16]]
ggsave(filename = "./picture/chp11/plot16.png",plot = p[[16]], height = 7, width = 10, dpi = 300)
```

---
## <h1 lang="en">fill = interaction()</h1> 
<img src="./picture/chp11/plot16.png" width="100%">

---
## <h1 lang="en">APA theme</h1> 
```{r plot 17, eval=FALSE, warning=FALSE}
# APA 主题
p[[17]] <- p[[16]] +
  ggplot2::theme_bw() + # 主要是为了去掉背景的灰色, 
  ggplot2::theme(legend.position = "none",
                 axis.text = element_text(size = 12),
                 axis.title = element_text(size = 14),
                 strip.text = element_text(size = 14),
                 plot.title = element_text(size = 16),
                 strip.background = element_blank(), # 去掉facet标题底色
                 panel.grid.major=element_blank(), # 隐藏主要网格线
                 panel.grid.minor=element_blank(), # 隐藏次要网格线
                 panel.border=element_blank(), # 隐藏边框
                 text=element_text(family='Times New Roman'), # 字体调整为Times New Roman
                 # 使用windowsFonts()查看字体对应的名字是什么
                 # 如果没有，可以导入 extrafont 包 library(extrafont)
                 # 这个包很笨，只能添加一个文件夹内的字体，但是默认字体文件夹内"C:/WINDOWS/FONT/"字体太多了,
                 # 因此建议你把需要的字体放到桌面，然后导入font_import(path = "C:/user/desktop/")
                 # 列出已导入的字体 fonttable()，使用FamilyName就可以了
                 ) 
names(p)[17] <- "APA theme"; p[[17]]
ggsave(filename = "./picture/chp11/plot17.png", plot = p[[17]], height = 6, width = 8, dpi = 300)
```
...
---
class: center, middle
<span style="font-size: 60px;">11.2 Tips</span> <br>

---
## <h1 lang="en">ase</h1> 
```{r df3 Summary, eval=FALSE, warning=FALSE}
df[[3]] <- df[[1]] %>%
  dplyr::group_by(Shape, Match) %>%
  dplyr::summarise(RT_mean = mean(RT)) %>%
  dplyr::ungroup() %>%
  dplyr::arrange(Match, Shape) %>%
  dplyr::mutate(RT_mean = round(RT_mean, 2))
names(df)[3] <- "Summary"
```

- 其实ggplot2的每一行代码，都代表一个独立的图片(当然这不包括theme这样的代码，主要指geom这样的) <br>
- 我们一般都会在第一行写ggplot2::ggplot(data = ?, ase(x = ?, y = ?)) <br>
- 这让我们错觉，ggplot的 "+" 就是dplyr中的 "%>%"，我们后面的每一行是对上一行得到的对象的修饰 <br>
- 但其实我们是对同一个对象画不同的图，然后重叠 <br>
- 这意味着，在每一行画图代码中，我们都可以设定data和aes <br>
- 而第一行ggplot(data = ?, ase(x = ?, y = ?))其实是在设定坐标轴， <br>
- 第一行实际的意义是，如果后面的画图不再指定data和aes，就默认使用第一行的data和aes <br>
<br>
为了让解释这一点，我们创建了df[[3]]，一个简单的八行三列的数据集

---
## <h1 lang="en">ggplot2::geom_text</h1> 
```{r plot 18, eval=FALSE, warning=FALSE}
# geom_text
p[[18]] <- p[[17]] +
  ggplot2::geom_text(data = df[[3]], aes(x = Shape, y = 0.9, label = RT_mean), 
                     hjust = 0, family='Times New Roman') 
names(p)[18] <- "geom_text"; p[[18]]
ggsave(filename = "./picture/chp11/plot18.png", plot = p[[18]], height = 6, width = 10, dpi = 300)
```

<img src="./picture/chp11/plot18.png" width="80%">

---
## <h1 lang="en">[aes(fill = ?)] & [aes(),fill= ?]</h1> 
```{r plot 19_fill out, eval=FALSE, warning=FALSE}
# fill 在aes里面和外面的区别
p[[19]] <- ggplot2::ggplot(df[[2]], aes(x = Shape, y = RT_mean), fill = interaction(Shape, Match)) + # fill的颜色基于Shape和Match
  tidybayes::stat_halfeye() +
  ggplot2::coord_flip() +
  ggplot2::scale_fill_manual(values = c(palette$sunset, palette$glassdoor)) + # 需要八种颜色来填充图片
  ggplot2::facet_wrap(~Match, ncol = 1) 

names(p)[19] <- "tidybayes::stat_halfeye"; p[[19]]
ggsave(filename = "./picture/chp11/plot19_out.png", plot = p[[19]], height = 6, width = 10, dpi = 300)
```

<img src="./picture/chp11/plot19_out.png" width="80%">

---
## <h1 lang="en">[aes(fill = ?)] & [aes(),fill= ?]</h1> 
```{r plot 19_fill in, eval=FALSE, warning=FALSE}
# fill 在aes里面和外面的区别
p[[19]] <- ggplot2::ggplot(df[[2]], aes(x = Shape, y = RT_mean, fill = interaction(Shape, Match))) + # fill的颜色基于Shape和Match
  tidybayes::stat_halfeye() +
  ggplot2::coord_flip() +
...
names(p)[19] <- "tidybayes::stat_halfeye"; p[[19]]
ggsave(filename = "./picture/chp11/plot19_in.png", plot = p[[19]], height = 6, width = 10, dpi = 300)
```

<img src="./picture/chp11/plot19_in.png" width="80%">

---
## <h1 lang="en">Width</h1> 
```{r plot 20 less, eval=FALSE, warning=FALSE}
# 相对大小, 想调整每个水平下图片的间距。需要同时调整ggplot中的width和ggsave中的width
p[[20]] <- ggplot2::ggplot(df[[2]], aes(x = Shape, y = RT_mean, fill = interaction(Shape, Match))) + 
  ggplot2::geom_boxplot(width = 0.1, position = position_nudge(x = -0.05), alpha = 0.5, outlier.color = NA, color = "grey") + 
  ggdist::stat_halfeye(adjust = 0.2,justification = -0.05, alpha = 0.5, width = 0.5, .width = 0, point_colour = NA,) +
  ggdist::stat_dots(side = "left",justification = 1.25, alpha = 0.5, width = 0.5) + 
  # 调整stat_dots的width会让图片之间相关更宽，但代价是点的数量变少了
  ggplot2::coord_flip() +
  ggplot2::scale_fill_manual(values = c(palette$sunset, palette$glassdoor)) + 
  ggplot2::facet_wrap(~Match, ncol = 1) +
  theme(legend.position = "none")
names(p)[20] <- "Plot Width"; p[[20]]
ggsave(filename = "./picture/chp11/plot20_less.png", plot = p[[20]], height = 6, width = 10, dpi = 300)
```

---
## <h1 lang="en">Width</h1> 

<img src="./picture/chp11/plot20_less.png" style="width:100%;">

---
## <h1 lang="en">Width</h1> 
```{r plot 20 more, eval=FALSE, warning=FALSE}
# 相对大小
p[[20]] <- ggplot2::ggplot(df[[2]], aes(x = Shape, y = RT_mean, fill = interaction(Shape, Match))) + 
  ggplot2::geom_boxplot(width = 0.1, position = position_nudge(x = -0.05), alpha = 0.5, outlier.color = NA, color = "grey") + 
  ggdist::stat_halfeye(adjust = 0.2,justification = -0.05, alpha = 0.5, width = 0.5, .width = 0, point_colour = NA,) +
  ...
  theme(legend.position = "none")
names(p)[20] <- "Plot Width"; p[[20]]
ggsave(filename = "./picture/chp11/plot20_more.png", plot = p[[20]], height = 10, width = 10, dpi = 300)
# 我们可以在调整了width的基础上，再调整ggsave中的height，让点的数量和之前一致
```

---
## <h1 lang="en">Width</h1> 

<img src="./picture/chp11/plot20_more.png" style="width:70%;">
