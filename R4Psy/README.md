...
- `slides/libs/` and `practice/libs/`: local JS/CSS dependencies generated by render / 渲染生成的本地依赖库
...
- `slides/libs/` and `practice/libs/` (generated dependency bundles)
...
- Keep chapter HTML outputs in `slides/`.
  - `slides/` 章节HTML暂时保留。
- Keep local dependency folders generated with outputs (`slides/libs/`, `practice/libs/`).
  - 与输出配套的本地依赖目录（`slides/libs/`、`practice/libs/`）保留。
...
### Q2. Why do we keep `slides/libs/` and `practice/libs/`? / 为什么保留 `slides/libs/` 和 `practice/libs/`？

Current Rmd sources use `lib_dir: libs`, so rendered HTML depends on local dependency folders next to each source group.
Do not delete these folders unless you plan to re-render all corresponding HTML immediately.

当前 Rmd 源文件使用 `lib_dir: libs`，渲染后的 HTML 会依赖所在目录的本地依赖库。
除非准备立即重渲染对应 HTML，否则不要删除这些目录。
