...
---
## <h1 lang="en">A reproducible project at a glance</h1>
<br>
- 一个项目一个根目录（project root），避免同一项目出现多个拷贝。<br>
- 跟踪 source files：`.R`、`.Rmd`、`README.md`、分析脚本与文档。<br>
- 忽略 machine-specific / temporary / derived files（通过`.gitignore`管理）。<br>
- commit要小且可解释：一次提交只做一类修改，便于review与回滚。<br>
...
---
## <h1 lang="en">3.2.2 AI辅助学习工具</h1> 
<br>
- 本课程提供了一个AI学习助手，可以帮助你学习和使用Git/GitHub。<br>
- 你可以直接通过使用opencode里的模型，对它们进行可以提问，例如：<br>
  - "什么是版本控制？"<br>
  - "帮我解释本地版本控制和远程版本控制的区别"<br>
  - "如何在R Studio中创建Git仓库？"<br>
- 详细的AI使用指南请参考：`skills/git-version-control/STUDENT_GUIDE.md`<br>
...
---
## <h1 lang="en">3.3.3 Commit message quality</h1>
<br>
- 建议：一次commit只解决一个清晰问题（atomic commit）。<br>
- 建议格式：`动词 + 为什么`，例如 `fix: correct typo in README title`。<br>
- 避免：`update`、`misc`、`final version` 这类无法理解意图的信息。<br>
- 如果改动很多，拆成多个小commit再提交，更方便自己和他人排查问题。<br>
...
---
## <h1 lang="en">3.3.6 What to track vs ignore</h1>
<br>
- Track（建议纳入版本控制）: 代码、文档、配置、README。<br>
- Ignore（建议忽略）: 系统缓存、临时文件、自动生成中间结果、密钥信息。<br>
- 先写`.gitignore`再开始批量添加文件，能减少后续清理成本。<br>
- `.gitignore`是“边界规则”，不是“事后补救”。<br>
...
---
## <h1 lang="en">3.4 Default beginner branch workflow</h1>
<br>
- Step 1: 先同步`main`分支（pull/sync），确保本地基线是最新。<br>
- Step 2: 为单个任务创建短生命周期分支（例如`homework-456ZS`）。<br>
- Step 3: 在该分支上小步commit并push。<br>
- Step 4: 发起pull request，由中央仓库review后合并。<br>
- 这样做能减少冲突，也让每次修改的意图更清晰。<br>
...
- **AI辅助**：你也可以使用OpenCode里的模型，用自然语言让AI帮你解释和执行这些命令，例如直接问"如何用git clone克隆仓库？"<br>
...
## Common anti-patterns (and safer choices)

- 一次提交改太多（"update all"） -> 拆成小commit，便于回滚与review。<br>
- 不加区分地添加全部文件 -> 先检查变更，只提交本次任务相关文件。<br>
- 本地和GitHub网页同时改同一文件且不先同步 -> 先pull/sync再编辑。<br>
- 用zip包传项目 -> 使用fork + branch + PR保留完整历史。<br>
...
- 项目结构清晰（单一project root，命名一致）
- commit历史可读（小步提交，message清晰）
- `.gitignore`边界明确（source tracked, temporary ignored）
- 能完成fork/branch/PR协作流程
- README包含本次作业的关键说明

**相关资源**:

- 去年B站网友推荐的视频：https://www.youtube.com/watch?v=bUoN85QvC10
- AI辅助学习指南：`skills/git-version-control/STUDENT_GUIDE.md`

**注意事项**：
- 第一次推送可能需要设置access token，可在GitHub的Settings->Developer settings->Personal access tokens中创建。
- 遇到问题时，可以参考AI辅助学习指南，或使用OpenCode里的模型寻求帮助。
