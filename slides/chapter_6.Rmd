...
# 添加行
new_row <- data.frame(id = 4, name = "赵六", age = 27, score = 80, pass = TRUE)
df <- rbind(df, new_row)
```
]
...
result <- ungroup(summarise(group_by(mutate(df, new_col = old_col * 2), 
                       group_var), 
...
**注意：group_by() 后一定要用 ungroup() 取消分组！**
]
]
...
# 选择指定列
df2 <- df %>%
  dplyr::select(Site, Sex, age, DEQ, ALEX1:ALEX16)
...
df2 <- df2 %>%
  dplyr::mutate(
    ALEX_total = rowSums(dplyr::select(., starts_with("ALEX")))
  )

# 计算均分
df2 <- df2 %>%
  dplyr::mutate(
    ALEX_mean = rowMeans(dplyr::select(., starts_with("ALEX")))
  )
...
df_clean <- df_all %>%
  # 排除右利手被试
  dplyr::filter(Hand == "R") %>%
  # 只保留正确率0或1的试次（排除未按键或按错键）
  dplyr::filter(ACC == 0 | ACC == 1) %>%
  # 只保留有效反应时范围[200ms, 1500ms]内的试次
...
```r
# pivot_wider: 长数据 -> 宽数据
# names_from: 用哪列的值作为新列名
# values_from: 用哪列的值填充新列
...
df_spe <- files %>%
  ...
  bind_rows() %>%
  dplyr::filter(Hand == "R", ACC == 0 | ACC == 1,
                RT >= 0.2 & RT <= 1.5) %>%
  dplyr::group_by(Sub, Shape, Label, Match) %>%
...
.font-size-14[
💡 与LLM互动时，不要只说“帮我处理一下数据”，而要把**现状、目标、约束**讲清楚。
]

---

# 6.16 Prompt 1：先澄清问题，再开始规划
...
# df_spe <- files %>%
#   lapply(function(f) {
#     read.table(f, header = TRUE) %>% filter(Date != "Date")
#   }) %>%
#   bind_rows() %>%
#   filter(***, ACC == 0 | ACC == 1, RT >= 0.2 & RT <= 1.5) %>%
#   group_by(Sub, Shape, Match) %>%
#   summarise(mean_RT = ***) %>%
#   ungroup() %>%
...
#   select(Sub, Identity, ***) %>%
#   pivot_wider(names_from = "Identity", values_from = "***") %>%
#   mutate(moral_SPE = *** - ***)
```
]
