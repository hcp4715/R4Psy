...
<span style="font-size: 30px;">2026年4月23日</span> <br>
...
pacman::p_load(
  # 本节课需要用到的 packages
  here,skimr,quartets,GGally,showtext,bruceR,tidyverse,DataExplorer,DT,
  # 生成课件
...
knitr::include_graphics("picture/chp8/reliable_research.jpg")
```

.footnote[
-----
参考链接：[文献列表及状态](https://www.nature.com/collections/idajfifcfg); 
...
---
# <h1 lang="en" style="font-size: 50px;">本节课安排与目标</h1>
<br>
.font-size-14[
1. 继续巩固上节课的练习  
&emsp;&emsp;问卷数据处理  
&emsp;&emsp;实验数据批量导入与整理  

2. 完成本节课的三个练习  

3. 学会结合 `prompt` 与 `AI/OpenCode` 辅助完成练习流程
]

<br>
.font-size-14[
配套提示词手册：`prompt_guides/llm_prompt_guide.md`
]
...
```{r, out.width = "10px"}
head(mt_raw, 3)  # 使用 base R 的 head 函数
```
...
## 练习前说明
.font-size-14[
- 开始练习前，可先查看配套提示词手册：`prompt_guides/llm_prompt_guide.md`
- 如果不确定该选择什么图形，或在 `ggplot` / 数据探索代码中遇到报错，可以先参考手册中的对应 section，再开始练习。
]
