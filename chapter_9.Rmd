...
df.penguin <- bruceR::import(here::here('data', 'penguin', 'penguin_rawdata.csv')) %>%
  #第一行是读取数据
  dplyr::mutate(subjID = row_number()) %>%                         #生成一个被试编号
...
m1_summary <- broom::tidy(m1)     #对m1模型的摘要信息进行提取，并且赋予新的名称m1_summary
...
bruceR::model_summary(m1)    #心理学学者开发的包，输出方式也比较好用
```
]
]
]
...
```{r lm in bruceR}
<<<<<<< HEAD
m2 <- bruceR::regress(Temperature_t2 ~ Temperature_t1,  # 公式 这个是bruceR的一个输出方式，叫做regress
          data = df.penguin, ) # 数据
install.packages("MuMIn")
=======
m2 <- bruceR::regress(Temperature_t2 ~ Temperature_t1,  # 公式
          data = df.penguin) # 数据
>>>>>>> b24708b3dfa0e3fcf95cdaea0e41bf95762166b0
```
---
class: left, middle
...
# 8.1 *t*-test
## 8.1.5 bruceR::TTEST
* 如果偏好按传统方式使用*t*检验，推荐`bruceR::TTEST`函数（因为他的输出符合心理学人的习惯）
...
```{r,results='hide'}
bruceR::TTEST(
  data = df.penguin, # 数据 ###他还会输出效应量，比较厉害
  y = "Temperature", # 因变量
  x = "romantic"     # 自变量
) 
```
...
res2 <- bruceR::MANOVA(
  data = df,
  dv = "Temperature",  #只有一个因变量
  between = "climate") #被试间变量
```
