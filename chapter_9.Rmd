...
## 虚拟编码
contrasts(df.penguin$climate) = contr.treatment(unique(df.penguin$climate))
### contr.treatment本质上创建了一个矩阵
### 由于3个分组，所以矩阵为2列


## 建立回归模型
lm_temp = lm(Temperature ~ climate,data = df.penguin)
...
MANOVA(data = mt_raw,
       ...
       within = cc("Identity,Valence")) %>% capture.output() %>% .[c(33:37)]
...
```{r warning=FALSE}
MANOVA(data = mt_raw,subID = 'Sub',dv= 'RT',
       within = cc("Identity,Valence")) %>% capture.output() %>% .[c(15:31)]
```
...
<br>
.bigfont[
-   同组间方差分析一样，只能提供显著性而没有给出关于差异大小的信息（个体间差异同样无法估计）；
]
...
但在注意与分层回归(Hierarchical regression)进行区分，即逐步引入自变量到回归模型中，以检验每个自变量对因变量的影响是否独立于其他自变量
]
...
---
.size5[
统计中经常谈及**固定效应**和**随机效应**，但在HLM中一般会在截距和斜率上分别讨论这两种效应。下面以一个例子来认识一下HLM与两种效应：我们希望研究教师的工龄(Experience)与薪水(Salary)之间是否存在关系，因而在学校中随机抽取了5个学院后，在每个学院中随机抽取部分教师信息，具体数据如下表所示：]
<br>
...
-   反应时会不会在Identity和Valence两种条件下出现个体差异？（随机斜率）
...
.pull-left[
```{r echo=FALSE}
subs = mt_raw %>% group_by(Sub) %>% 
  summarise(mean = mean(RT)) %>% 
  .[c(1:3,40:44),] %>% pull(Sub)
 mt_raw %>% 
  filter(Sub %in% subs) %>% 
  ggplot(aes(x = as.factor(Sub),color = Match,y = RT))  + 
  geom_point(position = position_jitter(0.2),alpha = 0.2)+
  geom_boxplot(outlier.colour = 'grey') + bruceR::theme_bruce() 
```
...
mt_raw %>% 
  filter(Sub %in% subs) %>% 
  ggplot(aes(x = as.factor(Sub),color = Shape,y = RT))  + 
  # geom_point(position = position_jitter(0.2),alpha = 0.2)+
  geom_boxplot(outlier.colour = 'grey') + bruceR::theme_bruce() 
```
...
$$fit = lme4::lmer(data = , formula = DV ~ Fixed\_Factor + (Random\_intercept + Random\_Slope | Random\_Factor))$$
...
在建立模型后，我们希望知道固定效应大小是否显著，但由于多层线性模型中对于自由度的估计有多种方法，比较复杂，在lme4::lmer()中是没有提供显著性。
...
lmerTest是一个与lme4包兼容的包，主要用于对线性混合效应模型进行假设检验；其中也包含了lmerTest::lmer()函数，与lme4::lmer()不同的是，其结果报告了显著性（使用Satterthwaite分母自由）
