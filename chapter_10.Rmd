...
mod_7302_full <- stats::glm(data = df.match.7302,
                          formula = ACC ~ 1 + Identity * Valence,
                          family = binomial)
summary(mod_7302_full) %>%
...
##结果解读
```{r}
summary(mod_full) %>% capture.output() %>% .[c(22:27)]
```
...
train_index <- createDataPartition(df.match$Sub, p = 0.7, list = FALSE)
