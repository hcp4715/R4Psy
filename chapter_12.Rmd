...
pacman::p_load(
  # 本节课需要用到的 packages
  here, tidyverse, bruceR, ggplot2, patchwork, magick,
  # 生成课件
...
```
```{r echo=FALSE}
head(df.match.trial, 3) %>% DT::datatable()
```
...
```{r echo=FALSE}
head(df.match.subj, 3) %>% DT::datatable()
```
...
```{r echo=FALSE}
head(df.match.sum, 3) %>% DT::datatable()
```
...
include_graphics('./picture/chp12/ggplot.png')
```
...
include_graphics('./picture/chp12/gramma.png')
```
- 和PS类似，采用图层的设计方式；
...
```{r}
# 以柱状图为例
p1 <- ggplot(data = df.match.sum, aes(x = Identity, y = grand_mean_RT, fill = Valence)) +
  geom_bar(stat = "Identity",
           position = "dodge") +
  geom_errorbar(data = df.match.sum,
...
## 可选图层
```{r echo=FALSE, out.width='70%', fig.align='center'}
include_graphics('./picture/chp12/adjust.png')
```

```{r}
# 以柱状图为例
p2 <- p1 +
  scale_y_continuous(expand=c(0, 0), breaks = seq(0, 0.75, 0.25), limits = c(0, 0.75)) +
  labs(title = "Mean RT for match trials", y = "RT") +
  papaja::theme_apa()
```
...
## 同理可以得到ACC的图
```{r echo=FALSE}
p3 <- ggplot(data = df.match.sum, aes(x = Identity, y = grand_mean_ACC, fill = Valence)) +
  geom_bar(stat = "Identity",
           position = "dodge") +
  geom_errorbar(data = df.match.sum,
  ...
  labs(title = "Mean ACC for match trials", y = "ACC") +
  papaja::theme_apa()
...
include_graphics('./picture/chp12/facet.png')
```
...
p4 <- df.match.sum.long %>%
  ...
  labs(title = "Mean for match trials",
       x="Identity",
       y="mean")
```
...
include_graphics('./picture/chp12/basic.png')
```
...
```{r echo=FALSE, out.width='80%', fig.align='center'}
include_graphics('./picture/chp12/summary.png')
```
...
## 目标图片1
```{r echo=FALSE, out.width='70%', fig.align='center'}
include_graphics('./picture/chp12/target1.png')
```
...
## 保存图片
```{r}
ggsave(filename = "./picture/chp12/p1.pdf", 
       plot = s9, 
       width = 5, 
       height = 4)
```
...
target2 <- df.match.trial %>% 
  filter(Sub == sublist[1:12]) %>%
  mutate(Label = factor(Label, levels = c("moralSelf", "moralOther", "immoralSelf", "immoralOther")),
         Sub = factor(Sub))
...
```{r out.width="70%"}
# 查看原始数据
ggplot(target2.subj, aes(x = Label, y = RT_mean, group = Sub, color = Sub)) +
  geom_point() +
  geom_line() +
  labs(y = "RT") +
  papaja::theme_apa()
...
## 可视化
.panelset[
.panel[.panel-name[提取画图所需的斜率与截距]
```{r}
# 提取模型的截距和斜率并转为数据框
random_slopes <- as.data.frame(t(apply(ranef(model)$Sub, 1,function(x) fixef(model) + x)))

# 获得每个被试的预测线：Intercept + Slope * Label
pre_slope <- melt(apply(random_slopes, 1, function(x) x[1] + x[2]*1:4), value.name = "RT_mean")

names(pre_slope)[1:2] <- c("Label","Sub")

pre_slope$Label <- as.factor(pre_slope$Label)
levels(pre_slope$Label) <- levels(target2.subj$Label)

pre_slope$Sub <- as.factor(pre_slope$Sub)
```

.panel[.panel-name[figure]
```{r}
z1 <- ggplot(pre_slope, aes(x = Label, y = RT_mean, group = factor(Sub), color = factor(Sub))) +
  geom_line() +
  geom_point(data = target2.subj, aes(x = Label, y = RT_mean)) +
  labs(y = "RT") +
  facet_wrap(~Sub,nrow=3) +
  papaja::theme_apa() 
```
...
# 12.2 进阶——细节调整
## 问题3：使用云雨图呈现数据的分布状态
## 云雨图将不在PPT中出现，感兴趣的同学可以自行回到rmd文件运行chunk
## 目标图片3
```{r echo = F,out.width="80%", fig.align='center'}
