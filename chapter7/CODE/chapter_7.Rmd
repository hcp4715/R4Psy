...
<span style="font-size: 30px;">2023/04/03</span> <br>

---
class: center, middle
<span style="font-size: 60px;">7.1  T-test</span> <br>
---
# <h1 lang="zh-CN">回顾t检验基础知识</h1>
<br>
<center>
<h2 lang="en">One sample t-test：单样本t检验</h2>
</center>
<br>
<h4 lang="en">In the one sample t test, we compare the sample mean against a fixed value.</h4>
<br>
<h3 lang="en">The hypotheses:</h3>
<br>
<h3 lang="en">H0:The current sample is sampled from a population with μ of the current value.</h3>
<h3 lang="en">H1:The current sample is sampled from a population with μ that different from the current value.</h3>
<h3 lang="en">The significance level: α = 0.05</h3>
<br>
---
# <h1 lang="zh-CN">回归t检验基础知识</h1>
<br>
<h2 lang="en">Typical research questions are:：与某一特定值比较的三种情况（大于/小于或等于）</h2>
<br>
<h3 lang="en">1.whether the mean (m) of the sample is equal to the theoretical mean (μ)?</h3>
<br>
<h3 lang="en">2.whether the mean (m) of the sample is less than the theoretical mean (μ)?</h3>
<br>
<h3 lang="en">3.whether the mean (m) of the sample is greater than the theoretical mean (μ)?</h3>
<br>
...
<br>
<center>
<h2 lang="en">Two-sample t-test ：双样本t检验</h2>
</center>
<br>
...
# <h1 lang="zh-CN">回归t检验基础知识</h1>
<br>
<h2 lang="en">Dependent (Paired) t-test：配对样本t检验</h2>
...
<br>
<br>
<h2 lang="en">Independent t-test:独立样本t检验</h2>
<h3 lang="en">The independent samples t-test is used to compare the mean of two independent groups.</h3>
<br>
<h3 lang="en">Typical research questions are:</h3>
<h3 lang="en">whether the mean of group A (mA) is equal to the mean of group B (mB)?</h3>
<h3 lang="en">whether the mean of group A (mA) is less than the mean of group B (mB)?</h3>
<h3 lang="en">whether the mean of group A (mA) is greather than the mean of group B (mB )?</h3>
---
class: center, middle
<span style="font-size: 60px;">7.2 T-test process in R</span> <br>
---
# <h1 lang="zh-CN">为代码实现编造几个问题</h1>
<br>
<br>
<h2 lang="zh-CN">1. “在penguin数据中，参与者对于家的依恋水平是否小于/等于/大于均值水平(假设在总体水平上，人们对家庭的依恋水平(HOME)均值为3.5)？”</h2>
<br>
<h2 lang="zh-CN">2. “在penguin数据中，男女生在亲密关系经验(ECR)中的得分是否存在显著差异？”</h2>
<br>
<h2 lang="zh-CN">3. “在penguin数据中，参与者在不同时间的温度差异是否具有显著性？”</h2>
...
---
# <h1 lang="zh-CN">TTEST()：t检验的相关参数</h1>
![](fig/PROCESS_19.png)
---
# <h1 lang="en">bruceR::PROCESS</h1>
## <h2 lang="zh-CN">问题1CODE_单样本t检验</h2>
```{r}
...
---
# <h1 lang="en">bruceR::PROCESS</h1>
## <h2 lang="zh-CN">问题2CODE_独立样本t检验</h2>
```{r}
    result.PROCESS <- capture.output({   
    bruceR::TTEST(data=df.pg.mean, 
                y="ECR_mean",
...
writeLines(result.PROCESS, "../OUTPUT/PROCESS.md")
...
---
# <h1 lang="en">bruceR::PROCESS</h1>
## <h2 lang="zh-CN">问题3CODE_配对样本t检验</h2>
```{r}
   result.PROCESS <- capture.output({
   bruceR::TTEST(data=df.pg.mean, 
                 y = c("Temperature_t1",              
...
<h4 lang="zh-CN">该示例和课程设置的问题无关，实际中可结合研究假设进行改写</h4>
---
class: center, middle
...
<br>
<center>
<h2 lang="zh-CN">one-way ANOVA:单因素方差分析</h2>
</center>
<br>
<h3 lang="zh-CN">概述</h3>
<h4 lang="zh-CN">单因素方差分析可以看成基础统计中两样本t检验的一个推广，即要比较试验观测值的某个因变量按照一个分组变量（即“因素”）分组后，各组的因变量均值有无显著差异。设因素 A 将所有观测分为 m个组，且各次观测相互独立。</h4>
<br>
![](fig/PROCESS_4.png)
<br>
---
...
<br>
<br>
<h4 lang="zh-CN">多因素方差分析原理与单因素方差分析基本一致，即利用方差比较的方法，通过假设检验的过程来判断多个因素是否对因变量产生显著性影响。当有两个及以上因素对因变量产生影响时，使用多因素方差分析。多因素方差分析不仅要考虑每个因素的主效应，往往还要考虑因素之间的交互效应。</h4>
<br>
![](fig/PROCESS_7.png)
---
class: center, middle
<span style="font-size: 60px;">7.4 anova process in R</span> <br>
---
# <h1 lang="zh-CN">为代码实现编造几个问题</h1>
<br>
<br>
...
<br>
<h2 lang="zh-CN">2. “在match数据中，被试在不同身份(self vs other)与不同效价(good vs bad)的条件组合下反应时是否存在差异？”</h2>
<br>
---
...
df.match.raw <-  read.csv('../DATA/MS_rep_matchingTask_raw.csv',
                       header = T, sep=",", stringsAsFactors = FALSE)%>%
                 ...
                 dplyr::mutate(Morality = as.factor(Morality)) %>%
...
df_within <- df.match.mean%>%
  bruceR::MANOVA(subID="Subject",#被试id
...
df.match.within <- df.match.mean %>%
    ...
    dplyr::mutate(Morality = paste("A_", Morality, sep = ""),#将变量的名称进行修改转换，不生产新的变量
                  Identity = paste("B_", Identity, sep = "")) %>%
    tidyr::unite("Conds", Morality:Identity, sep = "&",remove=TRUE) %>%#将morality和identity组合名称起来生成一个新的变量"Conds"
...
#被试在不同身份(self vs other)与不同效价(good vs bad)的条件组合下反应时是否存在差异）
  ...
  res_rmANOVA_1 <- bruceR::MANOVA(data=df.match.within,
                   dvs="A_Bad&B_Other:A_Good&B_Self",
...
---
# <h1 lang="zh-CN">简单效应检验与多重比较</h1>
# <h2 lang="zh-CN">问题2的简单效应比较_CODE</h2>
```{r}
result.PROCESS <- capture.output({
...
<br>
<br>
<h2 lang="zh-CN">1. “在match数据中，男性和女性的反应时是否存在差异(请分别使用t检验和方差分析来解决这个问题)？”</h2>
<br>
<h2 lang="zh-CN">2. “在match数据中，假设每名被试只参加了不同身份和效价组合的其中一种实验条件，那么不同身份与不同效价的条件组合下的反应时水平是否存在差异？”</h2>
<br>
---
class: center, middle
<span style="font-size: 60px;">本章内容结束<br>