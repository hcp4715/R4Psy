...
# 也可以用基础包，输入路径读取
Q1 <- read.csv("D:/R/R4Psy/data/penguin/penguin_rawdata.csv")
...
# 选择变量,同样，这里同样存在多种方法
# dplyr
Q3 <- Q2 %>% select(health, num_range("phone", 1:9))
...
# Base R
phone_cols <- grepl("^phone", names(Q10))  # 正则匹配列名
Q10$phone_total <- rowSums(Q10[, phone_cols, drop = FALSE], na.rm = TRUE)

# Q10$phone_total <- rowSums(
  # Q10[,grepl('^phone',names(Q10))],
  # na.rm = TRUE
#)
sum(is.na(Q10$phone_total)) 
