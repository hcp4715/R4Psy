...
```{r simulate the relationship between effect size and power}
...
for (i in effect_size) { # 遍历所有的效应量
  ...
  for (j in 1:10000) { # 在遍历效应量的基础上运行10000次计算p值，用于后续计算power
    x <- rnorm(n = 20, mean = i, sd = 1) # 生成20个服从均值为某一效应量，标准差为1的正态分布的样本
...
for (i in alpha) { # 遍历所有的效应量
  ...
  idx <- idx + 1 # 每运行一次for循环，idx就加1
  for (j in 1:10000) { # 在遍历效应量的基础上运行10000次计算p值，用于后续计算power
    x <- rnorm(n = 20, mean = 0.5, sd = 1) # 生成20个服从均值为某一效应量，标准差为1的正态分布的样本
...
for (i in sample_size) { # 遍历所有的效应量
  ...
  for (j in 1:10000) { 
    x <- rnorm(n = i, mean = 0.5, sd = 1) # 生成20个服从均值为某一效应量，标准差为1的正态分布的样本
...
for (i in 1:sim) {
  ...
  cohen_ds[i] <- (mean(group1) - mean(group2)) / (sqrt(((n1 - 1) * sd(group1)^2 + (n2 - 1) * sd(group2)^2) / (n1 + n2 - 2)))
}
```
---
可以看到，这里我们的统计检验力只有
