...
Q2 <- Q1 %>%
  filter(site == "Bamberg") %>%  # 先筛选 Bamberg 站点
  slice(15:34)  # 再选取第15到34行
```
...
Q5 <- Q4 %>%
  select(health) %>%                  # 仅保留 health 列
  mutate(health = factor(health,      # 转换为因子
    levels = c("Poor", "Fair", "Good", "Very Good", "Excellent"),  # 定义顺序
    ordered = TRUE                    # 设为有序因子
  ))
```
---
...
library(stringr)
Q9 <- str_sub(Q5$health, start = 1, end = 1)
```
...
Q13 <- Q10 %>%  # 假设 Q10 包含 health 和 phone_total 列
  filter(
    health %in% c("Good", "Very Good", "Excellent"),  # 条件1：健康等级
    phone_total >= 36                                 # 条件2：phone_total ≥ 36
  )
```
