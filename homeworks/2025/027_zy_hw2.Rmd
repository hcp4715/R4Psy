...
#转化，完成后可重新查看数据类型检查
Q3 <- select(Q2, health, starts_with("phone"))
str(Q3)
...
Q6_1 <- filter(Q5 , health %in% c('Poor'))
Q6_2 <- filter(Q5 , health %in% c('Fair'))
```
---
7.用paste()函数将Q5中的元素连接成一个字符串，中间用英文逗号隔开，并输出为Q7(tips:可以使用"?"查看函数功能)
```{r homework 7}
?paste
Q7 <- paste(Q5 , Q5$health , collapse = ",")
print(Q7)
```
...
```{r homework 9}
Q9 <- Q5 %>%
  mutate(across(everything(), ~ str_sub(as.character(.), start = 1, end = 1)))
```
...
# Base R
Q10 <-mutate(Q4 , Num = row_number())
```

---
11.找出数据框Q10中健康在'Good'及以上的被试的编号，输出为Q11。
```{r homework 11}
Q11 <- filter(Q10,health %in% c("Good", "Very Good", "Excellent"))
```
...
Q13 <-filter(Q10,health %in% c("Good", "Very Good", "Excellent") & phone_total >= 36)
```
