...
setwd("C:/Users/lenovo/Desktop/R_Homework/R Code for Analyses")
### Packages ####
library(psych)
library(sjPlot)
library(lme4)
...
### sample size (supplemental)###
d1 <- 0.35     
alpha1 <- 0.05  
power1 <- 0.95 
type1 <- "two.sample" 
alternative1 <- "two.sided"  

# size effect
sample1_power <- pwr.t.test(d = d1, 
                            sig.level = alpha1,
                            power = power1,
                            type = type1,
...
### participant description analysis ###
sample_labels <- c("4" = "US Sample 1", "5" = "Japan Sample 1", 
                   "6" = "US Sample 2", "7" = "Japan Sample 2")
gender_labels <- c("1" = "Male", "2" = "Female", "3" = "Non-binary/Other")
edu_labels <- c("1" = "Some High School", "2" = "Completed High School",
                "3" = "Some College", "4" = "Bachelor's Degree",
                "5" = "Master's Degree", "6" = "Doctorate Degree")
race_labels <- c("1" = "White", "2" = "Black", "3" = "Latino",
                 "4" = "Asian", "5" = "Other Race", "6" = "Other Race", "7" = "Other Race")
...
generate_demographics_table <- function(data) {
  data %>%
    ...
    summarise(
      ...
      Female = round(sum(Gender == 2) / n() * 100, 1),
      Male = round(sum(Gender == 1) / n() * 100, 1),
      NonBinary = round(sum(Gender == 3) / n() * 100, 1),
      
      White = ifelse(first(sample) %in% c(4,6), round(sum(Race == 1) / n() * 100, 1), NA),
      Black = ifelse(first(sample) %in% c(4,6), round(sum(Race == 2) / n() * 100, 1), NA),
      Latino = ifelse(first(sample) %in% c(4,6), round(sum(Race == 3) / n() * 100, 1), NA),
      Asian = ifelse(first(sample) %in% c(4,6), round(sum(Race == 4) / n() * 100, 1), NA),
      OtherRace = ifelse(first(sample) %in% c(4,6), round(sum(Race == 5,6,7) / n() * 100, 1), NA),
      
      SomeHS = {
        pct <- round(sum(Education == 1) / n() * 100, 3)
        ifelse(pct < 0.01, 0.004, pct)
      },
      HighSchool = round(sum(Education == 2) / n() * 100, 1),
      SomeCollege = round(sum(Education == 3) / n() * 100, 1),
      Bachelors = round(sum(Education == 4) / n() * 100, 1),
      Masters = round(sum(Education == 5) / n() * 100, 1),
      Doctorate = round(sum(Education == 6) / n() * 100, 1)
    ) %>%
    mutate(Sample = sample_labels[as.character(sample)]) %>%
    select(
      Sample, N, Age_Mean, Age_SD,
      Female, Male, NonBinary,
      White, Asian, Black, Latino, OtherRace,
      SomeHS, HighSchool, SomeCollege, Bachelors, Masters, Doctorate
    )
}

final_table <- generate_demographics_table(mega)
...
slope_result <- emtrends(mega_interaction,
                         ~ BeliefsTotal,
                         var = "SocialContactReverse",
                         at = list(BeliefsTotal = c(SD1_below, SD1_above)))
