...
simulateDistribution_withLogNormNoise <- function(m1, m2, b, internalNoise, diffRight, diffLeft, reps, shift, metanoise, metaNpriorChoice, metaNpriorConf)
{
  reps = reps
  mode_m = mode_m
  ...
  for (i in 1:sLength){
    for (x in 1:sLength) {
      ...
      data$logr1 = data$r1
      data$logr1[data$r1>0] = log(data$r1[data$r1>0])
      data$logr1[data$r1<0] = log(abs(data$r1[data$r1<0]))
      data$r1Noise[data$r1>0] = rlnorm(sum(data$r1>0), meanlog = data$logr1[data$r1>0], sdlog = metanoise)
      data$r1Noise[data$r1<0] = -rlnorm(sum(data$r1<=0), meanlog = data$logr1[data$r1<0], sdlog = metanoise)
      ...
      if (metaNpriorChoice==TRUE){
        data$priorRight = pnorm(abs(data$r1Noise), mean=0, sd=m1*internalNoise)
      }else{
      ...
      if (metaNpriorConf==TRUE){
        data$weightedPriorRight = pnorm(abs(data$r1Noise), mean=0, sd=b*m2*internalNoise)
      }else{
      ...
      data$logr2 = data$r2
      data$logr2[data$r2>0] = log(data$r2[data$r2>0])
      data$logr2[data$r2<0] = log(abs(data$r2[data$r2<0]))
      data$r2Noise[data$r2>0] = rlnorm(sum(data$r2>0), meanlog = data$logr2[data$r2>0], sdlog = metanoise)
      data$r2Noise[data$r2<0] = -rlnorm(sum(data$r2<=0), meanlog = data$logr2[data$r2<0], sdlog = metanoise)
