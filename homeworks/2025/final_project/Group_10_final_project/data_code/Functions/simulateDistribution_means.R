...
simulateDistribution_means <- function(m1, m2, b, internalNoise, diffRight, diffLeft, reps)
{
  reps = reps
  mode_m = mode_m
  ...
  for (i in 1:sLength){
    for (x in 1:sLength) {
      ...
      data$weightedPriorRight = pnorm(abs(data$s1), mean=0, sd=b*m2*internalNoise)
      ...
      if (mode_m==2){
        data$confTargetRight = Re((data$weightedPriorRight*(1 + erf(data$s2/(b*internalNoise*sqrt(2))))) / (1 + (2*data$weightedPriorRight - 1)*(erf(data$s2/(b*internalNoise*sqrt(2))))))
