...
fit_yb_yl <- function(d){
  ...
  if (is.na(bias)||is.na(eta)||is.na(lambda)){
    aic <- NA
  }else{
...
estimateNoise <- function(d){
  ...
  aics <- c(nbnl_1$aic, ybnl_1$aic, nbyl_1$aic, ybyl_1$aic)
  aicNew <- aics[!is.na(aics)]
  
  etas <- c(nbnl_1$eta, ybnl_1$eta, nbyl_1$eta, ybyl_1$eta)
  etas <- etas[!is.na(aics)]
  
  bias <- c(nbnl_1$bias, ybnl_1$bias, nbyl_1$bias, ybyl_1$bias)
  bias <- bias[!is.na(aics)]
  
  lapses <- c(nbnl_1$lambda, ybnl_1$lambda, nbyl_1$lambda, ybyl_1$lambda)
  lapses <- lapses[!is.na(aics)]
  
  #deltas <- c(nbnl_1$aic, ybnl_1$aic, nbyl_1$aic, ybyl_1$aic) - min(c(nbnl_1$aic, ybnl_1$aic, nbyl_1$aic, ybyl_1$aic))
  #deltas <- c(nbnl_1$aic, ybnl_1$aic) - min(c(nbnl_1$aic, ybnl_1$aic))
  deltas <- aicNew - min(aicNew)
  ...
  #eta_1 <- sum(weights * c(nbnl_1$eta, ybnl_1$eta))
  #bias_1 <- sum(weights * c(nbnl_1$bias, ybnl_1$bias))
  #lapse_1 <- sum(weights * c(nbnl_1$lambda, ybnl_1$lambda))
  
  eta_1 <- sum(weights * etas)
  bias_1 <- sum(weights * bias)
  lapse_1 <- sum(weights * lapses)
