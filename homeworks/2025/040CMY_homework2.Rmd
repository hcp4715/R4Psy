...
library(dplyr)
# 假设需要选取站点为 "Tsinghua" 的第1到20行
Q2 <- Q1 %>%
  filter(site == "Tsinghua") %>%  # 替换为实际站点名称
  slice(1:20)                     # 替换为实际行号范围
# Base R
 Q2 <- Q1[Q1$site == "Tsinghua" & (1:nrow(Q1)) %in% 1:20, ]
```
...
#转化，完成后可重新查看数据类型检查
Q3 <- Q3 %>% mutate(across(matches("phone[1-9]"), as.numeric))
```
...
```{r homework 5}
Q5 <- Q4$health %>%
  factor(levels = c("Poor", "Fair", "Good", "Very Good", "Excelent"))
```
---
