...
getwd()
file_path <- "D:/A学习/R4Psyfork/data/penguin/penguin_rawdata.csv"

# 读取 CSV 文件并赋值给 Q1
Q1 <- read.csv(file_path)
...
# 将 Oxford 站点的第7到26行数据赋值给 Q2
Q2 <- Q1 %>%
  filter(Site == "Oxford") %>%  # 筛选 Oxford 站点
  slice(7:26)                   # 选取第7到26行
...
#转化数据类型
Q3 <- Q3 %>%
  mutate(phone9 = as.integer(phone9)) 
...
```{r}
Q13<- Q10 %>% 
  filter(health %in% c("Good", "Very Good", "Excelent") & phone_total >= 36)
Q13
```
