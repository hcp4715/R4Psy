...
# Base R
colnames(Q2)
selected_cols <- c('health',colnames(Q2)[base::startsWith(colnames(Q2),'phone')])
Q3 <- Q2[ , selected_cols]

# 查看数据类型
str(Q3)
##'health'，'phone1-8'都属于整数型变量；'phone9'属于字符型变量
...
```{r homework 5}
Q5 <- factor(Q4$health,levels = c('Poor','Fair','Good','Very Good','Excellent'), ordered = TRUE)
sort(Q5) # 排序
...
library(dplyr)
Q10 <- Q10 %>%
  mutate(phone_total = rowSums(select(., starts_with("phone"))))
...
```{r}
Q13 <- Q10 %>%
  filter(health %in% c("Good", "Very Good", "Excellent") & phone_total >= 36)
...
---
16.创建一个名为 func3
的函数，该函数接受一个整数x作为输入(默认值：x=10)，并返回 TRUE（如果输入是偶数）或FALSE（如果输入是奇数）。并检验该函数对输入 22 的返回值，输出为Q16。(tips:函数%%用于计算两个数相除的余数)
```{r}
func3 <- function(x = 10){
  if((x %% 2 == 0)){
    result = 'TRUE'
...
func4 <- function(x){
  if (!is.numeric(x) || x != as.integer(x)) {
    return("Input must be an integer")} #确保输入是整数
  else if (x > 0){
    return('Positive')}
  else if (x < 0){
    return('Negative')}
