...
Q10 <- Q10 %>%
  rowwise()%>%
  mutate(phone_total = sum(c_across(starts_with("phone")), na.rm = TRUE))
Q10
```
...
```{r}
Q13 <- Q10 %>%
  filter(health >= 3, phone_total >= 36)
Q13
```
...
```{r}
func2 <- function(x) {
  data_frame(mean = mean(x), sd = sd(x))
}
Q15 <- func(c(1, 2, 3, 4, 5))
Q15
```
