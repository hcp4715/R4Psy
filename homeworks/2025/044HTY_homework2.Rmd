...
# 也可以用基础包，输入路径读取
Q1 <- read.csv(file = 'D:/Git/projects/homeworks/第二次小作业/data/penguin/penguin_rawdata.csv')
Q1
```
...
sapply(Q3,class)
#  health      phone1      phone2      phone3      phone4      phone5 
#  "integer"   "integer"   "integer"   "integer"   "integer"   "integer" 
#     phone6      phone7      phone8      phone9 
#  "integer"   "integer"   "integer" "character" 
# 将phone9转变为integer类型
Q3 <- Q3 %>% 
  dplyr::mutate(phone9 = as.integer(phone9))
sapply(Q3,class)

# Base R
Q3 <- Q2[,c('health','phone1','phone2','phone3','phone4','phone5','phone6','phone7','phone8','phone9')]
# 查看数据类型
sapply(Q3,class)
#转化，完成后可重新查看数据类型检查
Q3$phone9 <- as.integer(Q3$phone9)
sapply(Q3,class)
```

---
4.将“health”中的数值改为文字（5= Excelent; 4 = Very Good; 3 = Good; 2 = Fair; 1 = Poor），并记录为Q4（tips：可参考chapter_6所学内容）(你人还怪好的)
```{r homework 4}
# 推荐用tidyverse进行操作
Q4 <- Q3 %>% 
  dplyr::mutate(health = recode(health,
                                '5'='Excellent',
                                '4'='Very Good',
                                '3'='Good',
                                '2'='Fair',
...
```{r homework 5}
Q5 <- Q4 %>% 
  dplyr::mutate(health = as.factor(health)) %>% 
  dplyr::mutate(health = factor(health,
...
library(stringr)
Q8 <- Q5 %>% 
  dplyr::mutate(health = as.character(health))
Q8 <- stringr::str_length(as.character(unlist(Q8)))
Q8
```
...
Q9 <- Q5 %>% 
  dplyr::mutate(health = as.character(health))
Q9 <- stringr::str_sub(as.character(unlist(Q9)),1,1)
Q9
```
