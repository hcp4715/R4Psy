...
Q1 <- read.csv("R4psy/data/penguin/penguin_rawdata.csv")
```
...
# Base R
Q3 <- Q2 %>%
  select(health, matches("phone[1-9]"))  # 选择 "health" 和 "phone1-phone9" 列
...
Q13 <- Q10 %>%
  filter(health %in% c("Good", "Very Good", "Excellent") & phone_total >= 36)  #没有满足的被试
```
...
func2 <- function(x) data.frame(mean = mean(x), sd = sd(x))
Q15 <- func2(c(1, 2, 3, 4, 5))   #mean=3,sd=1.58
```
