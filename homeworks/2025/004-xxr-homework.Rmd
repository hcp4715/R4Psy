...
# dplyr
library(dplyr)
Q2 <- Q1[Q1$站点 == "Isinghua", ][1:20, ]
# Base R
isinghua_data <- Q1[Q1$站点 == "Isinghua", ]
Q2 <- isinghua_data[1:20, ]
head(Q2)
```
...
#'data.frame':	20 obs. of  10 variables:
 #$ health: int  3 3 3 3 2 2 3 3 2 2 ...
 #$ phone1: int  2 3 4 4 2 5 5 4 4 4 ...
# $ phone2: int  1 4 2 4 2 4 4 4 4 4 ...
 #$ phone3: int  1 3 2 4 2 3 2 2 1 4 ...
 #$ phone4: int  1 4 4 4 2 4 4 4 4 4 ...
 #$ phone5: int  1 4 4 4 3 5 4 3 1 4 ...
 #$ phone6: int  1 4 4 4 3 4 4 4 1 4 ...
 #$ phone7: int  2 5 2 4 4 5 5 2 4 2 ...
 #$ phone8: int  1 4 4 4 4 4 4 4 4 3 ...
 #$ phone9: chr  "3" "4" "4" "4" ...
#3转化使用mutate(across())对字符型列进行转换，完成后重新查看数据类型检查
Q3 <- Q3 %>% mutate(across(where(is.character), as.numeric)) 

# 4再次查看数据类型以确认转换成功
str(Q3)
#输出为phone9: num  3 4 4 4 4 4 5 3 4 5 ...

# 选择变量,同样，这里同样存在多种方法
# dplyr
Q3 <- Q2 %>% 
  select(health, `phonel-9`) 
str(Q3)
Q3 <- Q3 %>% 
  mutate(across(where(is.character), as.numeric))
str(Q3)
# Base R
Q3 <- Q2[, c("health", "phonel-9")]
str(Q3)
Q3$health <- as.numeric(Q3$health)
Q3$`phonel-9` <- as.numeric(Q3$`phonel-9`)
str(Q3)
# 查看数据类型
str(Q3)
#转化，完成后可重新查看数据类型检查
...
Q5 <- data.frame(health = factor(Q4$health,
                                levels = c("Poor", "Fair", "Good", "Very Good", "Excellent"),
                                ordered = TRUE))
Q5 <- Q5[order(Q5$health), , drop = FALSE]
head(Q5)
```
---
...
```{r homework 9}
Q9 <- substr(as.character(Q5$health), start = 1, stop = 1)
Q9
```
...
library(dplyr)
Q10 <- Q10 %>%
  mutate(phone_total = rowSums(select(., contains("phone")), na.rm = TRUE))
head(Q10)
# Base R
...
```{r}
condition <- (Q10$health %in% c("Good", "Very Good", "Excellent")) & 
             (Q10$phone_total >= 36)
Q13 <- Q10[condition, ]
Q13
```
