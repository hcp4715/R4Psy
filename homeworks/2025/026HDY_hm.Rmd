...
# 用 here::here 函数进行查找
setwd("D:/R/R4Psy")
Q1 <- bruceR::import(here::here('data', 'penguin', 'penguin_rawdata.csv'))
# 也可以用基础包，输入路径读取
setwd("D:/R/R4Psy")
...
library(dplyr)
Q2 <- Q1 %>%
  filter(Site == "Oslo") %>%  # 先筛选站点
  slice(3:22)                 # 再选取第3 到第 22 行
# Base R
selected_site <- Q1[Q1$Site == "Oslo", ]
Q2 <- selected_site[3:22, ]
...
# Base R
Q3 <- Q2[, c("health", grep("^phone[1-9]", names(Q2), value = TRUE))]
# 查看数据类型
str(Q3)
#$ health: int  3 5 3 4 3 5 5 3 4 4 ...
#$ phone1: int  1 3 4 3 2 2 1 3 1 3 ...
#$ phone2: int  1 3 4 3 2 2 1 4 1 4 ...
#$ phone3: int  1 3 2 1 2 1 1 4 1 1 ...
#$ phone4: int  2 4 4 4 2 1 1 4 4 4 ...
#$ phone5: int  1 2 4 1 3 2 1 3 2 1 ...
#$ phone6: int  1 2 2 4 2 1 1 4 2 1 ...
#$ phone7: int  2 4 4 5 2 2 4 3 3 2 ...
#$ phone8: int  4 2 2 1 2 2 1 4 2 4 ...
#$ phone9: chr  "4" "4" "2" "5" ...
#转化，完成后可重新查看数据类型检查
Q3 <- Q3 %>% mutate(phone9 = as.numeric(phone9))
```
...
```{r homework 5}
Q5 <- factor(Q4$health,levels = c("Poor", "Fair", "Good", "Very Good", "Excellent"))
sort(Q5) # 排序
```
---
...
```{r homework 11}
Q11 <- Q10 %>%
  filter(health >= "Good") %>%  # 筛选 "Good", "Very Good", "Excellent"
  select(Num)                   # 筛选 编号
```
...
# dplyr
Q10<- Q10 %>% 
  mutate(phone_total = rowSums(select(., starts_with("phone")), na.rm = TRUE))  # 筛选 以phone开头的变量
# Base R
...
```{r}
Q13 <- Q10 %>%
  filter(health >= "Good"&   # 健康在Good及以上
         phone_total >= 36)  # phone总分≥36
```
...
func2 <- function(x) {
  ...
  data.frame(
    ...
    sd = sd(x, na.rm = TRUE)       # 计算标准差，忽略NA值
  )
}
