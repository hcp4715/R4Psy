...
Q2 <- Q1 %>%
  filter(Site == "Tsinghua") %>%  # 站点名为"Tsinghua"302002
  slice(3:22)
```
...
# 将health列赋值给Q5，并按顺序排序
Q5 <- Q4$health  # 从Q4中提取health列
...
# 同样存在多种方法
# dplyr
Q10_dplyr <- Q4 %>%mutate(Num = row_number())
# Base R
#Q10_baseR <- Q4
#Q10_baseR$Num <- 1:nrow(Q4)  # 创建从1到总行数的序列

# 以dplyr方法
Q10 <- Q10_dplyr

# 查看结果
head(Q10)  # 显示前几行数据，包含新的Num列
...
# 使用dplyr
Q11 <- Q10 %>%
  filter(health >= "Good") %>%  # 利用健康状态的有序因子特性
  pull(Num)                     # 提取Num列作为向量
...
# dplyr
Q10<- Q10 %>% 
  mutate(phone_total = rowSums(select(., matches("phone[1-9]")), na.rm = TRUE))
# 查看结果，当前下只展示了前六列的结果
head(Q10)
# Base R
...
```{r}
# 筛选健康在'Good'及以上且phone_total >= 36的被试
Q13 <- Q10 %>%
  filter(health >= "Good",  # 利用健康状态的有序因子特性
         phone_total >= 36)  # 总分条件
