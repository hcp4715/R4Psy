...
#未包含，使用绝对路径
Q1 <- read.csv("D:/practice/RStudio/R4Psy/data/penguin/penguin_rawdata.csv")
```
...
# dplyr
library(dplyr)
#筛选Kassel站点的第13-20行数据
Q2 <- Q1 %>%
  filter(Site == "Kassel") %>%
  slice(13:20)
# Base R
Q2 <- Q1[Q1$Site == "Kassel", ][13:20, ]
```
...
# Base R
phone_vars <- paste0("phone", 1:9)
Q3 <- Q2[, c("health", phone_vars)]
# 查看数据类型
sapply(names(Q3), function(var) {cat(var, "的数据类型: ", typeof(Q3[[var]]), "\n")})
#转化，完成后可重新查看数据类型检查
for (var in names(Q3)) {if (typeof(Q3[[var]]) == "character") {Q3[[var]] <- as.numeric(Q3[[var]])}}
```
...
# 推荐用tidyverse进行操作
library(tidyverse)
df <- data.frame(health = c(5, 4, 3, 2, 1))
df <- df %>%
  mutate(Q4 = case_when(
    health == 5 ~ "Excellent",
    health == 4 ~ "Very Good",
...
df <- df %>%
  mutate(Q5 = Q4)
# 将Q5转换为因子型，并指定顺序
df$Q5 <- factor(df$Q5, levels = c("Poor", "Fair", "Good", "Very Good", "Excellent"))
# 按照Q5进行排序
df <- df %>%
  arrange(Q5)
```
...
Q9 <- str_sub(df$Q5, start = 1, end = 1)
```
...
row_nums <- 1:n
# 将行号向量添加为Q4的新列
Q10 <- data.frame(Q4, Num = row_nums)
...
```{r homework 11}
Q11 <- Q10 %>%
  filter(Q4 %in% c("Good", "Very Good", "Excellent")) %>%
  pull(Num)
...
# dplyr
Q10 <- Q10 %>% 
  mutate(phone_total = rowSums(across(starts_with("phone"), as.numeric), na.rm = TRUE))
# Base R
...
Q13 <- Q10 %>%
  filter(Q4 %in% c("Good", "Very Good", "Excellent") & phone_total >= 36)
```
...
# 使用func2函数计算向量c(1, 2, 3, 4, 5)的均值和标准差
