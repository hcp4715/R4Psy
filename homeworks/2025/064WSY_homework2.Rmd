...
Q2 <- Q1 %>%  
  dplyr::filter(Site == "Kassel") %>% # 提取Kassel站点的数据
  slice(11:30) # 选取第11到30行
...
# dplyr
Q3 <- Q2 %>% 
  dplyr::select(health, starts_with("phone"))
...
# 将Q5转换为因子型，并指定因子水平顺序为从"Poor"到"Excellent"
Q5 <- base::factor(Q5, levels = c("Poor", "Fair", "Good", "Very Good", "Excellent"))
...
```{r homework 9}
Q9 <- stringr::str_sub(Q5, start = 1, end = 1)
...
```{r homework 11}
Q11 <- Q10 %>% 
  dplyr::filter(health %in% c("Good", "Very Good", "Excellent")) %>% 
  pull(Num)
...
# dplyr
# Q10 <- Q10 %>% 
#   dplyr::mutate(phone_total = rowSums(
...
```{r}
Q13 <- Q10 %>% 
  dplyr::filter(health %in% c("Good", "Very Good", "Excellent") & phone_total >= 36) 

Q13 # 好像 phone_total 没有≥36的
```
...
func2 <- function(x) {
  mean <- mean(x)
  sd <- sd(x)
  result_df <- data.frame(mean, sd)
  return(result_df)
}

# 计算向量c(1, 2, 3, 4, 5)的均值和标准差
...
19.编写一个函数 func6，接受一个正整数 n 作为输入，使用 for 循环计算 n 的阶乘（factorial），并返回结果。然后使用该函数将 5 的阶乘计算出来，输出为 Q21。（tips：for 循环可以用于重复执行多次运算，将累乘结果保存在一个变量中）
```{r}
# 定义func6函数
func6 <- function(n) {
  result <- 1
