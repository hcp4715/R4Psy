...
# Note: This is our newly added code.
...
## Use the author's custom functions
## Function for removing a question from long or wide data frames
remove_cap_fun <- function(df, capacity, cap_var = "question") {
  df_new <- df

  # Check if the specified capacity is a column name in the data frame 
  ...
  return(df_new)  # Return the modified data frame
}
...
clean_char_fun <- function(var) {
  ## Convert all characters in the input variable to lowercase
  var <- tolower(var)
  ## Use case_when to standardize the variable based on common errors
  ...
  return(clean_var) # Return the cleaned variable
...
# ps: the author's custom function does not fully refer to this code in subsequent drawings
...
table2 <- bind_rows(d_us_adults, d_gh_adults, d_th_adults, d_ch_adults, d_vt_adults) %>%
  mutate(country = factor(country, levels = levels_country), # Recode factors and categorical variables
         response_cat = recode_factor(response_cat,
                                      "no" = "no",
                                      "kind of" = "kind of",
                                      "yes" = "yes", 
                                      .missing = "missing data")) %>%
  count(country, response_cat) %>% # Count response categories
  complete(response_cat, nesting(country), fill = list(n = 0)) %>% # Fill missing values
  group_by(country) %>% # Group by country
  mutate(prop = n/sum(n, na.rm = T)) %>% # Calculate proportion for each response category
  ungroup() %>% # Ungroup
  select(-n) %>%
  spread(response_cat, prop) %>%
  janitor::adorn_pct_formatting(digits = 2) # Format as percentage
...
## Factor retention: parallel analysis
if (file.exists("data/pa_outcomes_dist_adults.RDS")) {
  
  pa_outcomes_dist_adults <- readRDS("data/pa_outcomes_dist_adults.RDS")
  ...
  saveRDS(pa_outcomes_dist_adults, file = "data/pa_outcomes_dist_adults.RDS")
}
...
## Bootstrapped congruence
if (file.exists("data/cong_df_adults_oblique.RDS")) {
  # if the file exists, read the RDS file
  
  cong_df_adults <- readRDS("data/cong_df_adults_oblique.RDS")
  
} else {
  
  # if the file does not exist, perform data processing and bootstrapping
  ...
  saveRDS(cong_df_adults, file = "data/cong_df_adults_oblique.RDS")
}
```
...
if (file.exists("data/pa_outcomes_dist_children.RDS")) {
  
  pa_outcomes_dist_children <- readRDS("data/pa_outcomes_dist_children.RDS")
  ...
  saveRDS(pa_outcomes_dist_children, file = "data/pa_outcomes_dist_children.RDS")
}
...
if (file.exists("data/cong_df_children_oblique.RDS")) {
  
  cong_df_children <- readRDS("data/cong_df_children_oblique.RDS")
  ...
  saveRDS(cong_df_children, file = "data/cong_df_children_oblique.RDS")
}
```
